...
# Optional Student-t emission (nu learned via Newton or fixed), and BMA over multi-start models.
...
def load_index():
    """Load TWII daily close, compute 20D realized volatility proxy."""
    try:
...
def digamma(x):
    # Simple digamma approximation (OK for optimizer step)
    r = 0.0
    while x < 7:
        r -= 1 / x
        x += 1
...
def t_pdf(x, mu, sigma, nu):
    ...
    z = (x - mu) / sigma
    return c * (1.0 + (z * z) / nu) ** (-(nu + 1) / 2) / sigma


def em_hmm(obs, K=2, seeds=(0, 1, 2), max_iter=100, tol=1e-5,
           emission="gauss", nu=5.0, learn_nu=False):
    """EM for K-state HMM with Gaussian or Student-t emissions (multi-start)."""
    ...
    for seed in seeds:
        ...
        for _ in range(max_iter):
            # --- E-step ---
            if emission == "t":
                B = np.stack([t_pdf(x, mu[k], std[k], nu) for k in range(K)], axis=1)
            else:
                def gpdf(xx, m, s):
                    s = max(s, 1e-6)
            ...
            alpha[0] = pi * B[0]
            c[0] = alpha[0].sum() + 1e-18
            alpha[0] /= c[0]

            for t in range(1, n):
                alpha[t] = (alpha[t - 1] @ A) * B[t]
                c[t] = alpha[t].sum() + 1e-18
                alpha[t] /= c[t]
            ...
            gamma = alpha * beta
            gamma /= gamma.sum(axis=1, keepdims=True) + 1e-18
            ...
            pi = gamma[0]
            A = xi.sum(axis=0)
            A = A / (A.sum(axis=1, keepdims=True) + 1e-18)
    ...
    if best is None:
        raise RuntimeError("EM failed to converge on all starts.")
...
def model_ensemble_bma(obs, Ks=(2, 3, 4), seeds=(0, 1, 2, 3), emission="gauss", nu=5.0, learn_nu=False):
    """Build HMM candidate set and compute BMA weights via -0.5*BIC evidence."""
    cand = []
    n = len(obs)
    for K in Ks:
        for sd in seeds:
...
def model_select(obs, Ks=(2, 3, 4), seeds=(0, 1, 2, 3), criterion="bic", emission="gauss", nu=5.0):
    """Select best model by AIC/BIC among K and multi-starts."""
    n = len(obs)
    cand = []
    for K in Ks:
        ...
        if emission == "t":
            k_params += 0  # if nu fixed, no extra param
...
def main():
    ...
    emission = cfg_risk.get("hmm_emission", "gauss")  # "gauss" or "t"
    ...
    if use_bma:
        ...
        best, _ = model_select(obs, Ks=Ks, seeds=(0, 1, 2, 3, 4), criterion=criterion, emission=emission, nu=nu0)
